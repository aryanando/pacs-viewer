...
REM Hasta Radiologi PACS Viewer Startup Script (Windows)
REM Starts OHIF viewer with custom Hasta Radiologi configuration
...
echo.
echo 🏥 Hasta Radiologi PACS Viewer Startup
echo =====================================
...
REM Start the development server with custom config
REM Using cross-env to set environment variables properly
cross-env NODE_ENV=development APP_CONFIG=config/%CONFIG_FILE% PORT=%VIEWER_PORT% yarn dev
